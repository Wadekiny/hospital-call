 ...
 */
#define SLAVENUM 2
extern uint8 solve_bed[2];
extern uint8 warning_bed[SLAVENUM];
extern uint8 ask_bed;
void uart3_test_handler(void)//电脑
{
    uint8 ch;
    UARTn_e uratn = UART3;
    if(UART_S1_REG(UARTN[uratn]) & UART_S1_RDRF_MASK)   //接收数据寄存器满
    {
        //用户需要处理接收数据
        ch = UART_D_REG(UARTN[uratn]);
        if (ch < SLAVENUM && warning_bed[ch] == 1) solve_bed[ch] = 1;
    }   
}

uint8 now_state = 0;
void uart4_test_handler(void)//从机
{
    uint8 ch;
    UARTn_e uratn = UART4;
    if(UART_S1_REG(UARTN[uratn]) & UART_S1_RDRF_MASK)   //接收数据寄存器满
    {
        //用户需要处理接收数据
        ch = UART_D_REG(UARTN[uratn]);
        if (ch == 0x01 || ch == 0x00) now_state = ch;
        warning_bed[ask_bed] = ch;
    }
}